...
async function checkAllPermissions(autoShowGuide = false) {
  ...
  try {
    ...
    // 检查 Android 14+ 全屏通知权限
    try {
      const fsResult = await PermissionChecker.checkFullScreenIntentPermission();
      hasFullScreenIntentPermission.value = fsResult.granted;
    } catch (e) {
    ...
    // 检查多任务卡片是否已加锁
    try {
      const taskLockedResult = await Preferences.get({ key: 'task_locked' });
      taskLocked.value = taskLockedResult.value === 'true';
    } catch (e) {
      console.log('检查多任务卡片加锁状态失败:', e);
    }
...
// 标记多任务卡片已加锁
async function markTaskLocked() {
  taskLocked.value = true;
  await Preferences.set({ key: 'task_locked', value: 'true' });
  await checkAllPermissions();
}

// 用户手动确认悬浮窗权限已开启（部分华为手机开关灰色，需手动确认）
async function confirmOverlayPermission() {
  hasOverlayPermission.value = true;
  await Preferences.set({ key: 'overlay_confirmed', value: 'true' });
  await checkAllPermissions();
}
      ...
      <div class="permission-guide-card">
        ...
        <div class="permission-guide-scroll">
          ...
          <div class="permission-item" @click="openOverlaySettings">
            ...
            <button v-else class="perm-confirm-btn" @click.stop="confirmOverlayPermission">已开启</button>
          </div>
